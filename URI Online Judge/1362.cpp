...
void solve() {
    int shirtCount, personCount;
    cin >> shirtCount >> personCount;

    Graph graph(1 + 6 + personCount + 1);
    for (int i = 0; i < 6; i++)
        graph.addEdge(0, 1 + i, shirtCount / 6);

    static vector<string> sizes = { "XS", "S", "M", "L", "XL", "XXL" };
    for (int i = 0; i < personCount; i++) {
        string as, bs;
        cin >> as >> bs;
        int a = find(sizes.begin(), sizes.end(), as) - sizes.begin();
        int b = find(sizes.begin(), sizes.end(), bs) - sizes.begin();
        graph.addEdge(1 + a, 1 + 6 + i, 1);
        graph.addEdge(1 + b, 1 + 6 + i, 1);
        graph.addEdge(1 + 6 + i, 1 + 6 + personCount, 1);
    }

    bool can = graph.maxFlow(0, 1 + 6 + personCount) == personCount;
