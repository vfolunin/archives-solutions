...
public:
    int maxScore(vector<vector<int>> &a) {
        Graph graph(1 + a.size() + 100 + 1);

        for (int y = 0; y < a.size(); y++) {
            graph.addEdge(0, 1 + y, 1, 0);
            for (int value : a[y])
                graph.addEdge(1 + y, a.size() + value, 1, 0);
        }

        for (int value = 1; value <= 100; value++)
            graph.addEdge(a.size() + value, 1 + a.size() + 100, 1, -value);
        
        return -graph.minCostMaxFlow(0, 1 + a.size() + 100).first;
    }
};